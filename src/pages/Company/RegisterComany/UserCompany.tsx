...
function UserCompany() {
  ...
  const fetchDetailData = async () => {
    ...
    if (data.code === 200) {
      ...
      form.setFieldsValue({
        ...
        manufacturingSector: data.manufacturingSector.map(
          (item: any) => item.code
        ),
  ...
  const { options: provinceOptions } = useGetOptions({
    api: getListProvinceAPI, // Fetch major categories
    queryKey: "provinceKeyOptions",
    labelValueType: ["provinceName", "provinceCode"],
  });
